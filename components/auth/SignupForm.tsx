...
export function SignupForm({ returnTo: returnToProp, onDone, hideSwitchLink }: { returnTo?: string; onDone?: () => void; hideSwitchLink?: boolean } = {}) {
  const router = useRouter()
  const search = useSearchParams()
  const returnTo = useMemo(() => returnToProp || search.get("returnTo") || "/onboarding", [returnToProp, search])
    ...
    <div className="w-full space-y-6">
      ...
      {hideSwitchLink ? null : (
        <div className="pt-6 border-t border-zinc-50 text-center text-sm text-zinc-500">
          ...
          <a className="text-blue-600 font-medium hover:underline" href={`/auth/login?returnTo=${encodeURIComponent(returnTo)}`}>
            Log in
          </a>
        </div>
      )}
